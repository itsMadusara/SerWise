...
<body>
<%
    Object profilePicObj = null;
    if (session.getAttribute("employeeId")==null && session.getAttribute("uid")==null){
        response.sendRedirect("/SerWise_war/Login/login.html");
    }else {
        profilePicObj = session.getAttribute("employeeId");
        if(profilePicObj == null){
            profilePicObj = session.getAttribute("uid");
        }
    }
    String profilePicName = profilePicObj.toString();
    ...
    <div class="change-password-popup" id="passwordDiv">
        <div class="change-password-popup-content">
            ...
            <form method="post" action="/SerWise_war/changePassword">
                ...
                <input type="text" name="currPassword" pattern="{4}[0,9]+" id="opt" placeholder="OTP" autocomplete="off" required><br>
